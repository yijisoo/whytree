...
6. **Pattern 5 second gate for How Downs** — Pattern 5 says "only move to structural exploration after the user names what they are losing." Add a second condition: "Only introduce How Downs if the user explicitly moves toward it — do not initiate the structural turn yourself."
...
7. **Phase 0a framing density** — The three-beat framing (mechanism + example + permission) may be too dense for confused first-time users. Consider making the example optional or delivering it after the user's first response, grounded in their situation.
...
8. **READING.md divergent-life pitch** — Add a trigger note for users whose trees show multiple genuine non-converging roots. The Paul Graham essay's section on "working on many things" is relevant but the current pitch is written for single-direction users.
...
13. **Tighter obligation routing for demo mode** — Phase 0 says "1–2 exchanges of flat affect" before naming. In demo on a host's machine, even one flat exchange is enough signal — the social pressure on the guest is higher. Worth tightening for demo only.
...
14. **Name the meta-challenge pattern** — AI skeptics surface a recurring move: brief acknowledge, redirect to experience, no defense. Could be named alongside the existing pushback patterns in `PROBE_PATTERNS.md`.
...
15. **Suppress proactive feedback in demo mode** — Eager demo participants shouldn't be interrupted with feedback prompts. Add an explicit demo-mode note: suppress proactive feedback offers unless a clear tool-misfire occurred.

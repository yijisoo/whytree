...
- **익명 사용 핑** — 세션마다 한 번, "익명 ID가 N번째 세션, 첫 세션으로부터 X일째 도구를 사용했다"는 짧은 메시지만 보냅니다. 그게 전부입니다. 노드 수, 트리 깊이, 라벨, 단어 — 어떤 내용도 포함되지 않습니다. 트리를 측정하려는 게 아니라, 사람들이 계속 돌아오는지를 보려는 것입니다. (이전에 구조 정보(노드 수·깊이)를 포함한 옛 버전으로 동의하셨다면, 새로운 범위가 명확하도록 한 번 더 여쭤봅니다.)

...
# 3. Session notes (last 3 days)
echo "=== SESSION_NOTES ==="
if [ -f ~/.whytree/.current ]; then
  SLUG=$(cat ~/.whytree/.current)
  NOTES_DIR=~/.whytree/sessions/"$SLUG"
  if [ -d "$NOTES_DIR" ]; then
    CUTOFF=$(date -v-3d +%Y-%m-%d 2>/dev/null || date -d "3 days ago" +%Y-%m-%d 2>/dev/null || echo "0000-00-00")
    for f in $(ls -r "$NOTES_DIR"/*.md 2>/dev/null | head -5); do
      FNAME=$(basename "$f" .md)
      if [ "$FNAME" \>= "$CUTOFF" ] 2>/dev/null || [ "$FNAME" = "$CUTOFF" ]; then
        echo "--- $FNAME ---"
        cat "$f"
        echo
      fi

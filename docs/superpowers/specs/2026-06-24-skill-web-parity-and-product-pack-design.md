...
- The final name, parameters, and `focus_area` seeding rule of the `set_focus` initial-context operation must be resolved before Spec 1 ships its core (or `set_focus` is split into its own step); it must not remain open while listed as a Spec 1 deliverable.
